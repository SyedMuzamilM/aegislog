import { type ActorContext, type TenantContext, type AegisContext, logger as defaultLogger, runWithContext, type AegisLogger } from 'aegislog';
...
export async function withAegisContext<T>(
  ...
  const requestId = options.requestId ?? `act_${Date.now().toString(36)}_${Math.random().toString(36).slice(2, 7)}`;
  const tags = {
    ...options.tags,
    ...
    async () => {
      ...
      try {
        ...
        if (options.actionName) {
          ...
          log.info(`[ServerAction:Success] ${options.actionName} in ${duration}ms`, { durationMs: duration });
        }
        return result;
      } catch (error) {
        if (options.actionName) {
