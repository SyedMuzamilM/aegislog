...
export interface FastifyAegisOptions {
  ...
  getTenant?: (req: FastifyRequest) => TenantContext | undefined | Promise<TenantContext | undefined>;
  logRequests?: boolean;
}
...
async function aegisFastifyPluginFn(
  ...
  fastify.addHook('onRequest', (req: FastifyRequest, _reply: FastifyReply, done: (err?: Error) => void) => {
    const requestId =
    ...
    const actorPromise = options.getActor ? Promise.resolve(options.getActor(req)) : Promise.resolve(undefined);
    const tenantPromise = options.getTenant ? Promise.resolve(options.getTenant(req)) : Promise.resolve(undefined);
...
export const aegisFastifyPlugin: FastifyPluginAsync<FastifyAegisOptions> = fp(aegisFastifyPluginFn, {
