...
describe('AegisLog AI Tracker', () => {
  ...
  it('redacts sensitive PII in AI prompts and messages', async () => {
    ...
    await logger.ai.track({
      ...
      messages: [
        {
          role: 'user',
          content: 'My credit card is 4111 2222 3333 4444 and my auth key is sk-1234567890abcdef1234567890abcdef',
        },
      ],
      call: async () => ({ text: 'OK' }),
    });

    const startLog = memory.entries.find((e) => e.message.includes('[AI:Start] anthropic:claude-3-5-sonnet'));
  ...
  it('captures errors and logs them with latency', async () => {
    ...
    const errorLog = memory.entries.find((e) => e.message.includes('[AI:Error] openai:gpt-4o failed'));
